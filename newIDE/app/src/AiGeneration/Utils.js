  ...
  React.useEffect(
    () => {
      async function fetchSuggestionsIfNeeded() {
        ...
        console.log(lastMessage, shouldGenerateSuggestions);
        ...
        const isLastFunctionCallOutputProjectInitialization =
          functionCallOutputs.length > 0
            ? functionCallOutputs[
                functionCallOutputs.length - 1
              ].call_id.indexOf('initialize_project') !== -1
            : false;
        ...
        try {
          ...
          const aiRequestWithSuggestions = await getAiRequestSuggestions(
            getAuthorizationHeader,
            {
              ...
              suggestionsType: isLastFunctionCallOutputProjectInitialization
